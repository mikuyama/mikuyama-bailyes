...
const makeSocket = (config) => {
    ...
    const onUnexpectedError = (err, msg) => {
        ...
        // handle decryption / session errors gracefully - try re-upload prekeys
        if (message.includes('decryption') || message.includes('decrypt') || message.includes('signal')) {
            try {
                uploadPreKeysToServerIfRequired()
                    .catch(e => logger.warn({ e }, 'failed to re-upload prekeys after decryption error'));
            }
            catch (_e) {
                // ignore
            }
    ...
    const end = (error) => {
        ...
        // Signal if it's a bad session (401=loggedOut, 403=forbidden) vs just a restart needed
        const isBadSession = statusCode === Types_1.DisconnectReason.loggedOut ||
            statusCode === Types_1.DisconnectReason.forbidden;
        const isRestartRequired = statusCode === Types_1.DisconnectReason.restartRequired;
        ev.emit('connection.update', {
            ...
            ...(isBadSession && { isBadSession: true }),
            ...(isRestartRequired && { isRestartRequired: true })
        });
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(async () => {
        if (!lastDateRecv) {
            lastDateRecv = new Date();
        }
        ...
        /*
            ...
            increased tolerance to avoid false disconnects on slow networks
        */
        if (diff > keepAliveIntervalMs + 15000) {
            missedPings++;
            logger.warn({ diff, missedPings }, 'keepAlive: server not responding');
            ...
            // reset missed ping counter on successful contact
            try {
                await query({
                    tag: 'iq',
                    attrs: {
                ...
                missedPings++;
                logger.warn({ trace: err?.stack, missedPings }, 'error in sending keep alive ping');
                if (missedPings >= MAX_MISSED_PINGS) {
                    missedPings = 0;
                    end(new boom_1.Boom('Connection was lost', { statusCode: Types_1.DisconnectReason.connectionLost }));
                }
    ...
    ws.on('CB:stream:error', (node) => {
        ...
        const isBadSession = statusCode === Types_1.DisconnectReason.loggedOut ||
            statusCode === Types_1.DisconnectReason.forbidden;
        if (isBadSession) {
            logger.warn({ statusCode, reason }, 'bad session detected via stream error');
            ev.emit('connection.update', { isBadSession: true });
        }
    ...
    ws.on('CB:failure', (node) => {
        ...
        const isBadSession = reason === Types_1.DisconnectReason.loggedOut ||
            reason === Types_1.DisconnectReason.forbidden;
        if (isBadSession) {
            logger.warn({ reason }, 'bad session detected via CB:failure');
            ev.emit('connection.update', { isBadSession: true });
        }
