...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendPollMessage: async (jid, name, values, selectableCount = 1, options = {}) => {
        ...
        editMessage: async (jid, message, newText) => {
            const content = typeof newText === 'string' ? { text: newText } : newText;
            return await relayMessage(jid, {
                protocolMessage: {
                    key: message.key,
                    type: 14,
                    editedMessage: {
                        conversation: typeof newText === 'string' ? newText : (newText.text || '')
                    }
                }
            }, {
                messageId: Utils_1.generateMessageID()
